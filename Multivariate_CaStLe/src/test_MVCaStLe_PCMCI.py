"""
Script for testing multivariate CaStLe-PCMCI on 2D SCM(VAR) data.

This script evaluates the performance of the multivariate CaStLe-PCMCI algorithm on 2-dimensional
Structural Causal Model (SCM) data with Vector Autoregression (VAR). It loads spatial coefficients and data,
computes the true graph based on the dataset's coefficients, applies the CaStLe-PC algorithm to reconstruct
...
SAVE_PATH_DIR = SAVE_PATH_DIR[:-5] + "/results_castle_pcmci/"
...
results = ms.mv_CaStLe_PC(
    data, parcorr, pc_alpha, cd_function="run_pcmci", fdr_method=FDR_METHOD, rows_inverted=True, graph_p_threshold=pval_threshold
)
end_time = time.time()
