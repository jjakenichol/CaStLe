classdef Transient_ADR_2D < handle
    ...
    properties
        ...
        velocity_type       % Type of velocity field ("sinusoidal" or "constant").
        velocity_value      % Value of the constant velocity (2 x 1).
        reactant            % Reaction coefficient (default = 0.01).
        reaction_scaling    % Scaling factor for amount of a species becomes the other.
        init_concentration  % Initial concentration magnitude (default = 50).
        init_center         % Center of the initial condition blob (2 x 1).
        plume_size          % Size of the initial plume (default = 50).
    ...
    methods (Access = public)
        ...
        %% Constructor
        function this = Transient_ADR_2D(model, init_center, plume_size, diffusion_coeffs, advection_coeffs, velocity_field_type, velocity_parameters, reaction_coeff, reaction_scaling, nodes, init_concentration)
            ...
            %   Type of velocity field ("sinusoidal" or "constant").
            % velocity_parameters : [float, float]
            %   Value of the constant velocity (only used if velocity_type is "constant").
            ...
            arguments
                model
                init_center
                plume_size {mustBePositive} = 50  % Default plume size
                diffusion_coeffs {mustBePositive} = [0.05, 0.05]
                advection_coeffs {mustBeNumeric} = [1.00, 1.00]
                velocity_field_type {mustBeText} = "constant"
                velocity_parameters {mustBeNumeric} = [1.0, 0.0]
                reaction_coeff {mustBePositive} = 10
                reaction_scaling {mustBeNumeric} = 1.0
                nodes = 16
                init_concentration {mustBePositive} = 50  % Default initial concentration
            end
            ...
            this.velocity_type = velocity_field_type;
            this.velocity_value = reshape(velocity_parameters, 2, 1);
            this.reactant = reaction_coeff;
            this.reaction_scaling = reaction_scaling;
            this.init_center = reshape(init_center, 2, 1);
            this.plume_size = plume_size;  % Assign the plume size
            this.init_concentration = init_concentration;  % Save initial concentration
            this.model = model;
        end

        %% Problem definition
        function [out] = Initial_Plume(this, x, y)
            ...
            % out = 50 * exp(-this.plume_size .* sum(([x; y] - this.init_center).^2, 1)) + 1;
            % out = this.init_concentration * exp(-this.plume_size .* sum(([x; y] - this.init_center).^2, 1)) + 1;

            % Calculate squared distance from center point
            squared_distance = sum(([x; y] - this.init_center).^2, 1);
            % Apply Gaussian formula with parameters
            out = this.init_concentration * exp(-this.plume_size .* squared_distance) + 1;
        end
        ...
        function [v] = Velocity(this, x, y)
            ...
            if strcmp(this.velocity_type, "sinusoidal")
                xx = reshape(x, [], 1);
                xmin = min(xx);
                xmax = max(xx);
                xspan = xmax - xmin;
                ...
                % Constant velocity
                n = length(x);
                v = repmat(this.velocity_value', n, 1);
            end
        end

        function [r] = Reaction(~, y1, y2)
            % Reaction term between species.
            %
            % Models the conversion of species y1 into species y2.
            % The base reaction rate is scaled by the reactant coefficient.
            %
            % Parameters
            % ----------
            % y1, y2 : concentrations of species 1 and 2
            
            % Linear decay of species 1
            r = -y1;
            
            % Note: The actual decay applied will be this.reactant * r,
            % and the conversion to species 2 will be -this.reaction_scaling * this.reactant * r
        end
    ...
    methods (Access = protected)
        ...
        function [u0] = Initial_Condition(this, loc)
            ...
            M = length(loc.x);
            y1_0 = this.Initial_Plume(loc.x, loc.y);
        ...
        function [out] = ReactionTerm(this, loc, state)
            % Reaction term.
            % Computes how species convert between each other.
            % Species 1 converts to species 2 at a rate controlled by:
            % - reactant: overall reaction rate
            % - reaction_scaling: how much of species 1 becomes species 2
            %
            % Parameters
            % ----------
            % loc
            ...
            % Get the base reaction rate
            reaction = this.Reaction(state.u(1, :), state.u(2, :));
            
            % Apply reaction coefficients for both species
            % Species 1 decreases, species 2 increases (with scaling)
            out = this.reactant .* [reaction; -this.reaction_scaling*reaction];
        end
    ...
    methods (Static)
        ...
        function [model] = model_default(H, radius, mesh_shape)
            ...
            % ----------
            % H : float
            %   Maximum and minimum spacing in the spatial mesh. Smaller H means a
            ...
            arguments
                H {mustBePositive} = 0.05
                radius {mustBePositive} = 1.0
                mesh_shape {mustBeText} = "square"
            end

            % Initialize PDE object.
            model = createpde(2);

            if strcmp(mesh_shape, "square")
                % Set up the (square) geometry.
                geometryFromEdges(model, @squareg);
                generateMesh(model, "Hmax", H);
            else
                % Set up the (circular) geometry
                R1 = [1. 0. 0. radius]'; % [shape_type, center_x, center_y, radius]
                gd = R1;
                ns = char('R1');
                ns = ns';
                sf = 'R1';
                geometryFromEdges(model, decsg(gd, sf, ns));
                generateMesh(model,"Hmin",H,"Hmax",H);
            end
