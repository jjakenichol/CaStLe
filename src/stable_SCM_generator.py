"""
This script provides functions to generate stable Structural Causal Models (SCMs) for spatiotemporal datasets.
It includes utilities for creating random positive-semidefinite matrices, padding matrices, generating coefficient matrices,
checking matrix stability, and generating spatiotemporal datasets. Additionally, it provides functions to convert coefficient matrices
to graph structures compatible with the TIGRAMITE library and the CausalNex library.
...
Functions:
----------
1. multivariate_gaussian(pos, mu, Sigma)
    Returns the multivariate Gaussian distribution on array pos.

2. get_random_positive_semidefinite_matrix(size, Sigma)
    Generates a random positive-semidefinite matrix with given standard deviation.

3. pad_n_cols_right_of_2d_matrix(arr, n, fill_value)
    Adds n columns of zeros to the right of a 2D numpy array matrix.

4. pad_n_rows_below_2d_matrix(arr, n, fill_value)
    Adds n rows of zeros below a 2D numpy array matrix.

5. create_coefficient_matrix(local_coefficients, grid_size, fill_value=0.0)
    Generates a (grid_size**2, grid_size**2) size matrix from a given (3, 3) matrix for a toroidal space.

6. create_nonwrapping_coefficient_matrix(local_coefficients, grid_size=3, fill_value=0.0)
    Generates a (grid_size**2, grid_size**2) size matrix from a given (3, 3) matrix for a non-toroidal space.

7. is_stable(matrix, verbose=0)
    Checks for numerical stability, i.e., if all eigenvalues of the given matrix are < 1.0.

8. get_random_stable_coefficient_matrix(grid_size, density, min_value_threshold=None, verbose=0)
    Generates a random local dependence coefficient matrix that will produce a stable dynamics matrix.

9. get_graph_from_coefficient_matrix(dynamics_matrix, func=lambda x: x, return_val_matrix=False)
    Converts a dynamics matrix into a TIGRAMITE compatible graph object.

10. generate_dataset(T, grid_size, spatial_coefficients=None, dependence_density=None, min_value=None, error_sigma=0.1, error_mean=0, verbose=0)
    Generates a spatiotemporal dataset based on a given grid size and spatial coefficients.

11. get_graph_from_structure_model(structure_model, include_val_matrix=True)
    Converts a causalnex.structure.StructureModel to a string-graph and val_matrix in the style of the TIGRAMITE library.

Example Usage:
--------------
To generate a spatiotemporal dataset:
>>> dataset = generate_dataset(T=100, grid_size=5, dependence_density=0.5, min_value=0.1)

To convert a dynamics matrix to a TIGRAMITE compatible graph:
>>> graph = get_graph_from_coefficient_matrix(dynamics_matrix)

To convert a CausalNex StructureModel to a TIGRAMITE compatible graph:
>>> graph, val_matrix = get_graph_from_structure_model(structure_model)
"""
        ...
        for C_rowcol in range(grid_size):
            for C_colrow in range(grid_size):
                for C_colcol in range(grid_size):
                    ...
                    if ((padded_coeff_center[0] - offset[0]) >= grid_size) or ((padded_coeff_center[0] - offset[0]) < 0):
                        # No wrapping, reached a boundary and must ignore
                        continue
                    if ((padded_coeff_center[1] - offset[1]) >= grid_size) or ((padded_coeff_center[1] - offset[1]) < 0):
                        # No wrapping, reached a boundary and must ignore
                        continue
        ...
        try:
            if is_stable(dynamics_matrix, verbose - 1):
                ...
                if np.abs(min_value_threshold / min_value_actual * operator_norm) < 1:
                    ...
                    if verbose:
                        min_val_met = np.min(np.abs(scaled_local_coefficients[np.nonzero(scaled_local_coefficients)])) >= min_value_threshold
                        print(
                            "All values > {}: {}".format(
                                min_value_threshold,
                                min_val_met,
...
def get_stencil_graph(stencil_matrix: np.ndarray, func: Callable[[float], float] = (lambda x: x), return_val_matrix: bool = False):
    """
    Converts a stencil matrix into a TIGRAMITE compatible graph object.

    This function takes a stencil matrix and converts it into a structural causal model (SCM) according to TIGRAMITE's link dict definition.
    The SCM is then converted into a graph. The neighborhood dependence matrix defines the dependence of neighboring grid cells relative to the center grid cell.

    Parameters
    ----------
    stencil_matrix : numpy.ndarray
        The stencil matrix defining the dependence between neighboring grid cells.
    func : function, optional
        A function defining the functional dependence between grid cells. By default, it is a linear dependence (lambda x: x).
    return_val_matrix : bool, optional
        Whether or not to return the value matrix along with the graph, by default False.

    Returns
    -------
    stencil : TIGRAMITE graph
        Matrix format of the graph with 1 for true links and 0 otherwise.
    val_matrix : numpy.ndarray, optional
        The value matrix representing the coefficients of the dependencies, returned if return_val_matrix is True.
    """

    stencil_center = 4
    stencil_matrix = np.asarray(stencil_matrix)
    ndm = stencil_matrix.flatten()
    ...
    if return_val_matrix:
        val_matrix = np.zeros(
            (
                stencil_matrix.shape[0] ** 2,
                stencil_matrix.shape[1] ** 2,
                2,
            )
        )
        for row in range(val_matrix.shape[0]):
...
def generate_dataset(
    ...
    if spatial_coefficients is None:
        assert (dependence_density is not None) & min_value is not None, "Since no spatial_coefficients were passed, dependence_density and min_value must be passed."
        spatial_coefficients = get_random_stable_coefficient_matrix(grid_size, dependence_density, min_value_threshold=min_value, verbose=0)
    ...
    for t in range(1, T):
        for row in range(ROWS):
            for col in range(COLS):
                ...
                data[row, col, t] = (
                    from_self + from_left + from_right + from_top + from_bottom + from_top_left + from_top_right + from_bot_left + from_bot_right + np.random.normal(mu, sigma)
