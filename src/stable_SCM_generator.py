...
def create_coefficient_matrix(local_coefficients, grid_size, fill_value=0.0):
    ...
    coefficient_matrix = np.full(
        (grid_size, grid_size, grid_size, grid_size), fill_value
    )
        ...
        for C_rowcol in range(grid_size):
            for C_colrow in range(grid_size):
                for C_colcol in range(grid_size):
                    ...
                    coefficient_matrix[C_rowrow, C_rowcol, C_colrow, C_colcol] = (
                        padded_coeff[
                            (padded_coeff_center[0] - offset[0])
                            % padded_coeff.shape[0],
                            (padded_coeff_center[1] - offset[1])
                            % padded_coeff.shape[1],
                        ]
                    )
...
def create_nonwrapping_coefficient_matrix(
    local_coefficients, grid_size=3, fill_value=0.0
):
    """A mapping function that generates a (grid_size**2, grid_size**2) size matrix from a given (3, 3) matrix.
...
def get_random_stable_coefficient_matrix(
    grid_size, density, min_value_threshold=None, verbose=0
):
    ...
    while 1:
        ...
        dynamics_matrix = create_coefficient_matrix(
            local_coefficients, grid_size, fill_value=0.0
        )
        ...
        try:
            if is_stable(dynamics_matrix, verbose - 1):
                ...
                if np.abs(min_value_threshold / min_value_actual * operator_norm) < 1:
                    ...
                    dynamics_matrix_ = create_coefficient_matrix(
                        scaled_local_coefficients, grid_size, fill_value=0.0
                    )
                            ...
                            np.min(
                                np.abs(
                                    scaled_local_coefficients[
                                        np.nonzero(scaled_local_coefficients)
                                    ]
                                )
                            )
                            >= min_value_threshold
                    ...
                    assert (
                        scaling_equality
                    ), "local_coefficnets was not scaled correctly!"
...
def get_graph_from_coefficient_matrix(
    dynamics_matrix, func=lambda x: x, return_val_matrix=False
):
    ...
    for row in range(dynamics_matrix.shape[0]):
        SCM[row] = [
            ((col, -1), dynamics_matrix[row][col], func)
            for col in range(dynamics_matrix.shape[1])
        ]
...
def generate_dataset(
    ...
    if spatial_coefficients is None:
        ...
        ) & min_value is not None, "Since no spatial_coefficients were passed, dependence_density and min_value must be passed."
    ...
    for t in range(1, T):
        for row in range(ROWS):
            for col in range(COLS):
                ...
                from_right = (
                    spatial_coefficients[1, 2] * data[row, (col + 1) % ROWS, t - 1]
                )
                from_top = spatial_coefficients[0, 1] * data[row - 1, col, t - 1]
                from_bottom = (
                    spatial_coefficients[2, 1] * data[(row + 1) % ROWS, col, t - 1]
                )

                from_top_left = (
                    spatial_coefficients[0, 0] * data[row - 1, col - 1, t - 1]
                )
                from_top_right = (
                    spatial_coefficients[0, 2] * data[row - 1, (col + 1) % COLS, t - 1]
                )
                from_bot_left = (
                    spatial_coefficients[2, 0] * data[(row + 1) % ROWS, col - 1, t - 1]
                )
                from_bot_right = (
                    spatial_coefficients[2, 2]
                    * data[(row + 1) % ROWS, (col + 1) % COLS, t - 1]
                )
