...
def generate_dependent_data(
    parent_data: np.ndarray, dependence_function: Callable, initialization="zeros"
) -> np.ndarray:
    """Generates a 2D gridded spatiotemporal dataset that is dependent on another grid."""

    if initialization == "zeros":
        new_data = np.zeros(parent_data.shape)
    else:
        raise NotImplementedError("'initialization' only implemented for 'zeros'.")

    new_data = dependence_function(parent_data, new_data)
    return new_data
