"""
Script for testing Cartesian U-CaStLe-PC on 2D SCM(VAR) data.

This script evaluates the performance of the Cartesian U-CaStLe-PC algorithm on 2-dimensional
...
U-CaStLe-PC algorithm to reconstruct the graph, and evaluates the reconstruction's accuracy
against the true graph.

Cartesian U-CaStLe-PC runs independent U-CaStLe on each variable's spatial field (PC-Stable),
then combines intra-variable stencils with inter-variable center-to-center links discovered by
non-spatial PC on spatially aggregated timeseries.
"""
...
SAVE_PATH_DIR = SAVE_PATH_DIR[:-5] + "/results_cart_ucastle_pc/"
...
# Compute stencil with Cartesian U-CaStLe-PC
pc_alpha = 0.1
pval_threshold = 0.1
