"""
Script for testing Cartesian U-CaStLe-PCMCI on 2D SCM(VAR) data.

This script evaluates the performance of the Cartesian U-CaStLe-PCMCI algorithm on 2-dimensional
Structural Causal Model (SCM) data with Vector Autoregression (VAR). It loads spatial coefficients
and data, computes the true graph based on the dataset's coefficients, applies the Cartesian
U-CaStLe-PCMCI algorithm to reconstruct the graph, and evaluates the reconstruction's accuracy
against the true graph.

Cartesian U-CaStLe-PCMCI runs independent U-CaStLe on each variable's spatial field (PCMCI),
then combines intra-variable stencils with inter-variable center-to-center links discovered by
non-spatial PCMCI on spatially aggregated timeseries.
"""
...
SAVE_PATH_DIR = SAVE_PATH_DIR[:-5] + "/results_cart_ucastle_pcmci/"
...
# Compute stencil with Cartesian U-CaStLe-PCMCI
pc_alpha = 0.1
pval_threshold = 0.1
parcorr = ParCorr(significance="analytic")
start_time = time.time()
results = cartesian_ucastle_pc(
    data,
    parcorr,
    pc_alpha,
